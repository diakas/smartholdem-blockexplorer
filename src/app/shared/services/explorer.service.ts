...
export class ExplorerService {
  ...
  public getStandby(n: number): Observable<any> {
    ...
    return this.http.get(`${this._network.NODE}/delegates?offset=64`)
      .map((res: Response) => res.json());
  }
  ...
  public getActiveDelegates(nextForgers, blocks): Observable<any> {
    ...
    return this.http.get(`${this._network.NODE}/delegates/?orderBy=rate:asc&limit=64`)
  ...
  public getLatestBlocks(): Observable<any> {
    return this.http.get(`${this._network.NODE}/blocks?orderBy=height:desc&limit=64`)
      .map((res: any) => res.json().blocks)
  ...
  public getNextForgers(): Observable<any> {
    return this.http.get(`${this._network.NODE}/delegates/getNextForgers?limit=64`)
  ...
  private _getDelegatesByPublicKey(): Observable<any> {
    return this.http.get(`${this._network.NODE}/delegates?limit=64`)
      .map((res: any) => res.json())
      .flatMap((res: any) => {
        const requests = [];
        ...
        for (
          let index = 0;
          index < Math.ceil(res.totalCount / 64);
          index++
        ) {
          requests.push(
            this.http.get(`${this._network.NODE}/delegates?limit=64&offset=${index * 64}`)
          );
        }
